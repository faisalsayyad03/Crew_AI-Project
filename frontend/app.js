...
async function submitRecommendation(event) {
    ...
    // Validate key (unless provider is kimi, which has a backend fallback key)
    if (!apiKey && provider !== "kimi") {
        showToast("Please enter an LLM API Key to run CrewAI agents.", "error");
